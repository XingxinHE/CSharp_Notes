...
2.Use `checked` and `unchecked` (recommend✅)
...
|                                  | C#               | C++                |
| -------------------------------- | ---------------- | ------------------ |
| Access to manage object lifetime | ❌              | ✅ |
...
- `disposing == true`
  - release <u>large, managed resource</u>✅ and <u>unmanaged resource</u>✅
  - used by `public` version
- `disposing == false`
  - don't release <u>large, managed resource</u>❌ and but release <u>unmanaged resource</u>✅
...
> ❌	The old and <u>**outdated**</u> way to do is:
...
> ✅	Add `delegate` outside of the class
...
> ✅	Create a **read/write property** to provide access to a `private` delegate
...
> ✅	Provide complete **<u>encapsulation</u>** by implementing separate `Add` and `Remove` methods
...
> ​	:two:Implement with `Task` and `Wait`, ❌
...
> ​	:three:Implement with `Task` and define continuation❌
...
​	does - ✅  specify that the code in the method can be divided into one or more continuations.

​	does not - ❌  signify that a method runs asynchronously on a separate thread

`await`:

​	does - ✅  specifies when the C# compiler can <u>split the code into a continuation</u>. The right hand side of `await` is an awaitable
...
> ​	**Solution 1:hammer:**         Will block until the task complete❌
...
The following is a <u>**bad**</u> example❌:
...
| --------------------- | ------------------ | ------------------ | ------------------------------ | ------------------------------ |
| simple exclusion lock | ✅ | ✅ | ❌                            | ❌                            |
| semaphores            | ❌                | ❌                | ✅(controlled) | ✅(controlled) |
| reader/writer locks   | ❌                | ✅ | ✅             | ❌                            |
